...
public class Day02_1 {
    ...
    public static void main(String[] args) {
        List<Integer> numbers = Arrays.stream(getInputValues())
                .map(Day02_1::getSumOfGames)
                .toList();
    ...
    private static Integer getSumOfGames(String a){
        int MAX_RED_CUBES = 12;
        int MAX_GREEN_CUBES = 13;
        int MAX_BLUE_CUBES = 14;
        int gameNumber = Integer.parseInt(a.substring(5,a.indexOf(':')));
        String[] sets = a.substring(a.indexOf(":")+2).split(";");
        Map<String, Integer> colorCubes;

        for(String set: sets) {
            String[] cubesRevealed = set.split(",");
            colorCubes = new HashMap<>();

            for (String colorNumber : cubesRevealed) {
                //c[1] -> color; c[0]-> number
                String[] c = colorNumber.trim().split(" ");
                if (colorCubes.containsKey(c[1])) {
                    int num = colorCubes.get(c[1]);
                    num += Integer.parseInt(c[0]);
                    colorCubes.put(c[1], num);
                } else {
                    colorCubes.put(c[1], Integer.parseInt(c[0]));
                }
            }

            if (colorCubes.getOrDefault("green",0) > MAX_GREEN_CUBES ||
                    colorCubes.getOrDefault("red",0) > MAX_RED_CUBES ||
                    colorCubes.getOrDefault("blue",0) > MAX_BLUE_CUBES) {
                return 0;
            }
        }
        return gameNumber;
