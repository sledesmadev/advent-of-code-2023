...
public class Day01_2 {
    ...
    public static void main(String[] args) {
        List<Integer> numbers = Arrays.stream(getInputValues())
                .map(Day01_2::changeStringForNumber)
                .toList();
    ...
    private static Integer changeStringForNumber(String s){
        ...
        return  getFirstAndLastNumber(x.toString());
    }

    private static Integer getFirstAndLastNumber(String a){
        String first = Arrays.stream(a.split("")).filter(Utils::isNumeric).findFirst().orElse("");
        String last = Arrays.stream(new StringBuilder(a).reverse().toString().split("")).filter(Utils::isNumeric).findFirst().orElse("");
